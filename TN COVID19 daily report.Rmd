...
For the most up-to-date numbers visit the [TN state Dept of Health](https://www.tn.gov/health/cedep/ncov.html) or local health department sites. In particular, the [Metro. Nashville site](https://www.asafenashville.org/) has a very nice summary- and note that their report of daily cases usually precedes the State Health Dept, due to the verification process and assignment of county of residence at the state.
...
-   [The Bloomberg vaccine tracker](https://www.bloomberg.com/graphics/covid-vaccine-tracker-global-distribution/) provides the best detail regarding doses allocated and dispensed.\
...
-   [COVID Tracking project](https://covidtracking.com/data#state-tn) (links data from TN DOH). This is where I get the US data.\
...
These data are taken directly from the [TN state Dept of Health](https://www.tn.gov/health/cedep/ncov/data/downloadable-datasets.html). I am using these data, by date of report, rather than date of test, for consistency.
...
```{r echo=FALSE}
total_cases <-  sum(age_death_table$ar_casecount)
total_deaths <-  sum(age_death_table$ar_totaldeaths)

age_death_table %>%   
  mutate(ar_case_pct = sprintf("%1.0f%%", ar_casecount/total_cases*100),
  ...
  select(age.f, populationtn_byage_2019, ar_casecount, ar_case_pct, ar_totaldeaths, ar_death_pct, ar_totaldeaths_100k, ar_totaldeaths_pct, ar_recipientvax_pct) %>%
  filter(!is.na(age.f)) %>% 
  htmlTable::htmlTable(total=F, align = "r", align.header = "r", rnames = F,
                       # caption = "Age Group Estimates",
                       header = c("Age Group", "Population (n)", "Cases", "Cases</br>(% of Total)", "Deaths", "Deaths</br>(% of Total)", "Deaths/100k", "Deaths (% in Age)", "Given 1st Vaccine (% in Age)"),
                       tfoot = paste0("<i>Population within each age group taken from US Census estimates for TN, 2019</br>Vaccination data last updated on ", mostrecent_vax_agedate, ".</i>"),
...
Counties are ordered by highest new cases per day (current) based on the most recent 7 day average (per 100k population). Max limit of the color scale set at `r n_ul = 200; n_ul` to increase contrast- some local outbreaks exceed this limit (eg prison outbreaks in Bledsoe, Lake, Wayne Co.)
...
Testing and Case data for these maps are taken from the [COVID Tracking project](https://covidtracking.com/data#state-tn), and the 2019 state population estimates are taken from the [United States Census Bureau](https://www.census.gov/programs-surveys/popest.html).
