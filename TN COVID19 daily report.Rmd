...
*Last Data Update: `r file.info(here::here("data_tndoh/Public-Dataset-County-New.xlsx"))$mtime`*. All source documents for this report can be found on [github](https://github.com/JMLuther/TN_COVID19)

This document updates weekly each Wednesday after the TN State DOH releases data (usually Wednesday at ~4PM). The CDC releases data each weekday on their [site](https://covid.cdc.gov/covid-data-tracker/#cases_casesper100klast7days).

For official, weekly TN State Dept of Health Updates, go to the [Critical Indicator Report](https://www.tn.gov/content/dam/tn/health/documents/cedep/novel-coronavirus/CriticalIndicatorReport.pdf).

# **VACCINE SIGN-UP**  

For a reliable discussion of COVID-19, vaccines, and emerging variants, listen to [Dr. Aronoff's Medicine Grand Rounds](https://www.youtube.com/watch?v=Vh8p6OJuE4U&t=37s&ab_channel=VUMCDepartmentOfMedicine) from August 2021.

**All ages over 6 months are now eligible** for COVID vaccination- Go to one of these sites to sign up:

  *  For ages 6mos to 5 years vaccine is available at VUMC Children's Hospital. [Go here to sign up](https://myhealthatvanderbilt.com/openscheduling/standalone?dept=101000419&vt=1400621221&stringkey=PedCovidVaccine) or call 615-936-5437 to schedule.  
  ...
  *  [Vanderbilt Medical Center sign-up](https://covid-vaccine.app.vumc.org/schedule/#/availability) with sites in Nashville, Shelbyville, and Tullahoma  
...
age_death_table %>%   
  ...
  select(age.f, populationtn_byage_2019, ar_totalcases, ar_case_pct, ar_totaldeaths, ar_cfr, ar_death_pct, ar_totaldeaths_100k, ar_totaldeaths_pct, 
         ar_recipientvax_pct
         ) %>%
  filter(!is.na(age.f)) %>% 
...
```{r echo=FALSE}
vax_by_age_mostrecent %>% 
  select(age.f2, recipient_count, populationtn_byage_2019_B, ar_recipientvax_pct) %>%
  mutate(ar_recipientvax_pct = sprintf("%1.2f%%", ar_recipientvax_pct)) %>% 
  # filter(!is.na(age.f)) %>% 
  htmlTable::htmlTable(total=F, align = "r", align.header = "r", rnames = F,
                       # caption = "Age Group Estimates",
                       header = c("Age Group", "Given 1st Vaccine (n)", "Estimated Pop. (n)", "Given 1st Vaccine (% in Age)"),
                       tfoot = paste0("<i>Population within each age group estimated from US Census estimates for TN, 2019</br>Vaccination data last updated on ", mostrecent_vax_agedate, ".</i>"),
                       css.cell = rbind(rep("font-size: 1.1em; padding-left: .5em; padding-right: .5em;", times=ncol(.)),
                                        matrix("padding:0 5px 0 5px;", ncol=ncol(.), nrow=nrow(.))))

```

*most recent County Vaccination data update on `r max(tndoh_vax_county_current$date)`*
...
**most recent County data update on `r max(tndoh_countynew$date)`**
...
The Variant distribution is reported by 10 US Regions. Only data for the US total and  Region 4 (SouthEast) is shown here. CDC [NowCast data](https://covid.cdc.gov/covid-data-tracker/#variant-proportions) is used for this report.


```{r interactive_variants_plot, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
pl_variants <-
  cdc_variants_data %>%
...
counties_top10_increase_full <-
  ...
  filter(date == max(date, na.rm = T),
         ...
         name %in% TN_counties$name)   # exclude the pending and out-of-state
counties_top10_increase_full_n <- length(counties_top10_increase_full$county)
counties_top10_increase <-
  counties_top10_increase_full %>% 
  filter(new_cases_county_7davg_1wkdelta > 0) %>% 
  top_n(n = 10, wt = new_cases_county_7davg_1wkdelta) %>%  
  pull(name)
```

Counties with the top 10 greatest weekly increase in New Cases, by % Change of 7-day average versus 7 days prior (limited to counties with average of \>5 daily cases). Case rates are currently increasing in `r ifelse(counties_top10_increase_full_n == 0, "no", counties_top10_increase_full_n)` counties.
...
`r ifelse(length(counties_near_peak_cases_top10) == 0, "No", length(counties_near_peak_cases_top10))` counties are currently near (at or within >90% of) all-time peak new case rate. 
...
plot_counties_near_peak
  # 
#    print("No counties currently at peak")
 # ifelse(length(counties_near_peak_cases_top10) == 0, "No", length(counties_near_peak_cases_top10))
...
```{r include=FALSE}
# FILTER CO WITH DECREASE
counties_top10_decrease_full <-
  tndoh_countynew %>%
  ungroup() %>%
  filter(date == max(date, na.rm = T),
       new_cases_county_7davg_1wkdelta < 0,
         name %in% TN_counties$name)   # exclude the pending and out-of-state
counties_top10_decrease_n <- length(counties_top10_decrease_full$county)
counties_top10_decrease <-
  counties_top10_decrease_full %>% 
  # filter(new_cases_county_7davg >= 5) %>% 
  top_n(n = -10, wt = new_cases_county_7davg_1wkdelta) %>%
  pull(name)    
```
...
Cases are currently declining in `r counties_top10_decrease_n` counties
...
tndoh_age_trends <-
  ...
  select(date, age.f, ar_newcases) %>% 
  group_by(age.f) %>% 
  nest(data = c(date, ar_newcases)) %>% 
  mutate(data = map(data, ~mutate(.x, 
                                  ar_newcases_ts = ts(ar_newcases, frequency = 7, start = 1),
                                  ar_newcases_trend.stl = stl(ar_newcases_ts, s.window = 7)$time.series[, "trend"])
         )) %>% 
  unnest(cols = c(data))

pl_newcases_age_2021 <-
  tndoh_age_trends %>% 
  filter(date >= lubridate::ymd("2021-06-01")) %>%
  # filter(date >= lubridate::ymd("2021-05-01"),
  #        age.f %in% c("31-40", "41-50","51-60", "61-70", "71-80", "81+")) %>% 
  ggplot(aes(x = date,
             y = ar_newcases_trend.stl,
             fill = age.f)) +
  geom_area(alpha = 0.5) +
  theme(legend.position = "none") +
  labs(x = NULL, y = NULL,
       title = "Current: Daily New Cases by Age Group") +
  scale_x_date(date_breaks = "2 month", date_labels = "%b-%y", expand = c(0, 0)) +
  facet_wrap(~age.f, nrow = 1) +
  theme(axis.text.x.bottom = element_text(angle = 90, size = 7))


y_labelpos <- max(layer_scales(pl_newcases_age_2021)$y$range$range[[2]])


# note: age groups need to be adjusted to calculate Vax % again

cowplot::plot_grid(pl_newcases_age_2021 + 
                     geom_text(data = age_death_table, size = 3, hjust = 0, vjust=1, fontface = "italic",
                               aes(x=lubridate::ymd("2021-06-01"), y = y_labelpos,
                                   label = sprintf("Vacc.:%1.1f%%", ar_recipientvax_pct) )),
                   pl_newcases_schoolage,
                   nrow = 2)
...
`r show_cdc <- exists("pl_us_cases_100k"); if(!show_cdc){"CDC Data Currently unavailable"}`

## New Cases

*mouse over state for details*

```{r us_newcases, fig.height=10, fig.width=12, eval=show_cdc}
...
```{r state_heatmap, fig.height=8, fig.width=10, eval=show_cdc}
...
**You made it to the end, so here's a random [XKCD comic](https://xkcd.com/) (please excuse any profanity- just randomly selected):**

<i> 
```{r echo=FALSE, fig.height=7, message=FALSE, warning=FALSE}
# PRIOR DAD JOKE VERSION
#___________________________________
# ```{r echo=FALSE, results='asis'}
# dadjoke::dadjoke() 
#___________________________________

# get XKCD metadata for most recent comic
xkcd.today <- jsonlite::parse_json(httr::GET(url = "https://xkcd.com/info.0.json"))
# get a random XKCD comic instead of today's
n <- sample(1:xkcd.today$num, 1)
xkcd.rand =   jsonlite::parse_json(httr::GET(url = paste0("https://xkcd.com/", n,  "/info.0.json")))
img_path <- here::here("code/xkcd.png")
download.file(url = xkcd.rand$img, img_path, mode = "wb") # download the actual image
# grid::grid.newpage()
# grid::grid.raster(png::readPNG(temp))
# g <- gridtext::textbox_grob(
#   paste(strwrap(xkcd.rand$alt), collapse = "<br>"),
#   x = unit(0, "npc"), y = unit(0, "npc"),
#   hjust = 0, vjust = 0, halign = 0.5,
#   gp = gpar(fontsize = 13),
#   box_gp = gpar(col = "grey", fill = "grey90"),
#   r = unit(5, "pt"),
#   padding = unit(c(2, 2, 2, 2), "pt"),
#   margin = unit(c(0, 2, 0, 2), "pt"))
# grid.draw(g)
knitr::include_graphics(img_path, dpi = 100)
