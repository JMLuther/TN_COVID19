...
If you are 16 years old or older, you are elgible to get your vaccine in Nashville (for free)- Go to the [Metro Public health site](https://www.asafenashville.org/) and click the link to schedule your appointment!
...
tndoh_age <- 
  left_join(tndoh_age,
          ...
          by = "age.f") |> 
  group_by(age.f) |> 
  mutate(ar_new_cases_100k = ar_newcases/populationtn_byage_2019*100000,
...
pl_new_tnsdc <-
  ...
  geom_rect(aes(xmin = max(tndoh_daily$date)-7, xmax = max(tndoh_daily$date) + 1, ymin = -Inf, ymax = Inf),
            fill = "grey90", alpha = 0.05, color = "grey", linetype = 3) +
...
age_death_table %>%   
  ...
  select(age.f, populationtn_byage_2019, ar_totalcases, ar_case_pct, ar_totaldeaths, ar_death_pct, ar_totaldeaths_100k, ar_totaldeaths_pct, ar_recipientvax_pct) %>%
  filter(!is.na(age.f)) %>% 
