...
tndoh_daily <-
  ...
  arrange(date) %>% 
  filter(date > lubridate::ymd("2000-01-01")) # State occasionally releases data with default date of "1960-01-01"for missing values (SAS)
...
tndoh_vax_age <- readxl::read_excel(here::here("data_tndoh/COVID_VACCINE_AGE_GROUPS.XLSX"),
                                      col_types = c("text", "date", rep("numeric", 9)))
...
vax_by_age_mostrecent <-
  ...
  filter(date == max(date, na.rm = T)) %>% 
    select(date, recipient_count, age.f2, ar_recipientvax_pct) 
